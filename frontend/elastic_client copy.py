...
class ElasticManager:
    def __init__(self, hosts, timeout=10, max_retries=5, retry_interval=2):
        ...
        self.active_alarms_index = "monitor_historical_alarms" # Consistent naming
        self.historical_alarms_index = "historical_alarms" # Consistent naming
                ...
                if index_name in [self.active_alarms_index, self.historical_alarms_index]:
                    mappings = {
                        "properties": {
                            ...
                            "group_id": {"type": "keyword"}, # For collective groups
                            "type": {"type": "keyword"} # e.g., 'ilo_device', 'windows_monitor', 'ird_channel', 'ird_overview', 'snmp_kmx'
                        }
                    }
        ...
        try:
            ...
            for alarm_data in current_active_alarms:
                ...
                if "type" not in alarm_data:
                    alarm_data["type"] = source_type

                # Generate a stable ID for active alarms to allow easy replacement/update
                # This ID should be unique per alarm type + unique alarm message
                stable_alarm_id_parts = [
                    alarm_data.get('type', 'generic_type'),
                    alarm_data.get('source', 'generic_source'),
                    alarm_data.get('server_ip', 'unknown_ip'),
                    alarm_data.get('device_name', 'unknown_device'),
                ...
                # Index into active alarms (with stable ID for replacement)
    ...
    def fetch_all_alarms(self, size=1000):
        """Fetches all alarms from the active_alarms index."""
        if not self.es:
        ...
        try:
            ...
            response = self.es.search(index=self.active_alarms_index, body=query, size=size)
            alarms = [hit['_source'] for hit in response['hits']['hits']]
            return alarms
        except Exception as e:
            logging.error(f"Error fetching all alarms from {self.active_alarms_index}: {e}")
            return []
    ...
    def clear_index(self, index_name):
        ...
        try:
            ...
            logging.info(f"Successfully cleared all documents from index '{index_name}'.")
            return True
        except Exception as e:
