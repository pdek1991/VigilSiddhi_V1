    ...
    async def check_pgm_routing_status(self, ip, community, pgm_dest_id, expected_router_source, domain):
        ...
        if status == "Success":
            if polled_source_clean == expected_router_source_clean:
                status = "OK"
                message = f"{domain_prefix}PGM Destination {pgm_dest_id} routing is correct. Polled: '{polled_source_clean}', Expected: '{expected_router_source_clean}'"
            else:
                status = "MISMATCH"
                message = f"{domain_prefix}PGM Routing MISMATCH for Destination {pgm_dest_id}. Polled: '{polled_source_clean}', Expected: '{expected_router_source_clean}'"
            ...
            message = f"{domain_prefix}SNMP Error for PGM Destination {pgm_dest_id}. Polled: '{polled_source_clean}'. {snmp_result.get('message', '')}"
