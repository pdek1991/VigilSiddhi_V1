...
class MySQLManager:
    ...
    def add_device_ip(self, ip_address, username, password, frontend_block_id, device_name):
        query = """
            INSERT INTO device_ips (ip_address, username, password, frontend_block_id, device_name)
            VALUES (%s, %s, %s, %s, %s)
        """
        params = (ip_address, username, password, frontend_block_id, device_name)
        return self._execute_query(query, params, commit=True)

    def get_all_device_ips(self):
        query = "SELECT ip_id, ip_address, username, password, frontend_block_id, device_name FROM device_ips"
        return self._execute_query(query, fetch_all=True)
    
    def get_device_ip_by_frontend_block_id(self, frontend_block_id):
        query = "SELECT ip_id, ip_address, username, password, frontend_block_id, device_name FROM device_ips WHERE frontend_block_id = %s"
        return self._execute_query(query, (frontend_block_id,), fetch_one=True)

    def update_device_ip(self, ip_id, ip_address, username, password, frontend_block_id, device_name):
        query = """
            UPDATE device_ips SET ip_address = %s, username = %s, password = %s, frontend_block_id = %s, device_name = %s
            WHERE ip_id = %s
        """
        params = (ip_address, username, password, frontend_block_id, device_name, ip_id)
        return self._execute_query(query, params, commit=True)
